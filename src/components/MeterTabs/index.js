...
const MeterTabs = ({ meter1, meter2, initialActiveTab, initialCurrentPage, initialYearFilter, initialMonthFilter }) => {
    ...
    // 🔹 Apply filters
    let data = meter.data;
    if (monthFilter) data = data.filter((row) => row.month.toLowerCase().startsWith(monthFilter.toLowerCase()));
    ...
    return (
        <>
            ...
            <div className={styles.consumerInfo}>
                <p><strong>Consumer ID:</strong> {meter.consumerId}</p>
                <p><strong>Reference No:</strong> {meter.referenceNo}</p>
                <Link href={"https://bill.pitc.com.pk/fescobill"} target="_blank" className={styles.link}>FESCO Web</Link>
            </div>

            {/* Filters */}
            <div className={styles.filters}>
                <select value={yearFilter} onChange={handleYearChange} className={styles.filterSelect}>
                    <option value="">All Years</option>
                    <option value="2025">2025</option>
                    <option value="2024">2024</option>
                </select>
                <input type="text" placeholder="Filter by month" value={monthFilter} onChange={handleMonthChange} className={styles.filterInput} />
            </div>
            ...
            <div className={styles.tableWrapper}>
                <table className={styles.table}>
                    ...
                    <tbody>
                        {currentRows.length > 0 ? currentRows.map((row, i) => (
                            <tr key={i}>
                            ...
                            <tr className={styles.noDataRow}><td colSpan="4">No Data Found</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
